...
from prior import BlockwiseUniform


def get_prior(lower, upper):
    return BlockwiseUniform(low=jnp.asarray(lower), high=jnp.asarray(upper))
...
def get_shear_experiment(data_dir):
    ...
    # Chi-2 minimistaion statistic
    def _mle(xi, pi):
        pi_ = mle(xi,
            pi=pi,
            Finv=Finv, 
            mu=mu + jnp.dot(pi - alpha, derivatives), 
            dmu=derivatives, 
            precision=precision
        )
        return pi_
...
def get_estimated_objects(key, n_sims, *, data_dir):
    """ 
    ...
    fiducials = jr.multivariate_normal(key, mean=mu, cov=covariance, shape=(n_sims,))
...
def get_experiment_data(key, true_covariance, n_sims, *, results_dir, data_dir):
    ...
    if true_covariance:
        ...
        ) = get_estimated_objects(key, n_sims, data_dir=data_dir) # NOTE: no cosmology needed here?
    ...
    X = jax.vmap(mle, in_axes=(0, 0, None, 0, None, None))(
        D, 
        Y, 
